clisi_information <- function(c_g, d_g, membership_vec, 
  ...
  c_g <- multiomicCCA:::.symmetrize_sparse(c_g, set_ones = T)
  d_g <- multiomicCCA:::.symmetrize_sparse(d_g, set_ones = T)
  
  # compute clisi scores
  cell_subidx <- multiomicCCA:::.construct_celltype_subsample(membership_vec, max_subsample_clisi)
...
.clisi_cell <- function(g, membership_vec, position, tol = 1e-3){
  ...
  neigh <- multiomicCCA:::.nonzero_col(g, position, bool_value = F)
  len <- length(neigh)
  if(len == 0){
    return(rep(0, k))
  }
