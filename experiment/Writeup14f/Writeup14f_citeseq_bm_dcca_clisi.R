...
max_val <- 1
k <- nrow(mat_c)
idx_list <- vector("list", 3)
idx_list[[1]] <- 1:k
idx_list[[2]] <- which(sort(unique(bm$celltype.l2)) %in% c("Prog_B 1", "Prog_B 2", "Prog_DC", "Prog_Mk", "Prog_RBC", "HSC", "LMPP", "GMP"))
idx_list[[3]] <-which(sort(unique(bm$celltype.l2)) %in% c("CD4 Memory", "CD4 Naive", "CD8 Effector_1", "CD8 Effector_2", "CD8 Memory_1", "CD8 Memory_2", "CD8 Naive", "Memory B", "Naive B"))
names(idx_list) <- c("All", "Progenitors", "Developed cells")
