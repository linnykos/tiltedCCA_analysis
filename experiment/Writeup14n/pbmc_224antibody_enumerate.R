...
source("../../main/pbmc_224antibody_colorPalette.R")
...
for(j in 1:nrow(param_df)){
  ...
  for(lap in c("laplacian_1", "laplacian_2", "common_laplacian")){
    tmp <- pbmc; tmp_mat <- tmp_obj$laplacian_list[[lap]]
    ...
    ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup14n/citeseq_pbmc224_", j, "_", lap, ".png"),
  ...
  png(paste0("../../../../out/figures/Writeup14n/citeseq_pbmc224_laplacian_heatmap-", j, ".png"),
  ...
  tiltedCCA:::plot_scores_heatmap.list(obj = list(lap_full),
                                       main = paste0("k: ", param_df$latent_k[j],
                                                     ...
                                                     "\nint: ", param_df$bool_intersect[j],
                                                     ", deg: ", param_df$min_deg[j]),
                                       membership_vec = as.factor(tmp$celltype.l2),
