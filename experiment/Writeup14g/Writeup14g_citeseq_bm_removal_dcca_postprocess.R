...
load("../../../../out/Writeup14g/Writeup14g_citeseq_bm_removal_dcca.RData")
...
rownames(common_umap@cell.embeddings) <- rownames(bm2@meta.data)
bm2[["dcca_common"]] <- Seurat::CreateDimReducObject(common_umap@cell.embeddings)
...
rownames(distinct1_umap@cell.embeddings) <- rownames(bm2@meta.data)
bm2[["dcca_distinct1"]] <- Seurat::CreateDimReducObject(distinct1_umap@cell.embeddings)
...
rownames(distinct2_umap@cell.embeddings) <- rownames(bm2@meta.data)
bm2[["dcca_distinct2"]] <- Seurat::CreateDimReducObject(distinct2_umap@cell.embeddings)
...
for(umap_name in other_names){
  print(umap_name)
  u_mat1 <- bm2[[anchor_name]]@cell.embeddings
...
file_vec <- c("rna", "tiltedcca-common", "tiltedcca-distinct1", "tiltedcca-distinct2")
...
for(i in 1:length(reduction_vec)){
  for(j in 1:length(group_vec)){
    ...
    plot1 <- plot1 + ggplot2::ggtitle(paste0("Removed CD4 and CD8a:\n", main_vec[i]))
    plot1 <- plot1 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
    ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup14g/Writeup14g_citeseq_bm_removal_", file_vec[i], ".png"),
