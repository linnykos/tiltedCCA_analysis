...
.kl_selection_lp <- function(num_neigh,
  ...
  obj_mat <- .generate_objmat_kl(obs_tab)
  ...
  obj_vec <- .generate_objvec_kl(num_neigh = num_neigh,
                                 obs_tab = obs_tab,
                                 prior_1 = prior_1,
                                 prior_2 = prior_2)
  
  tmp <- .generate_constr_kl(obs_tab,  
...
.select_kl_cells <- function(desired_tab, idx_df){
...
.generate_indices_kl <- function(bool_row,
                                 idx,
                                 p1, p2){
  if(bool_row) {
    idx + c(0:(p2-1))*p1
...
.generate_objmat_kl <- function(obs_tab){
  p1 <- nrow(obs_tab); p2 <- ncol(obs_tab)
  
  row_objmat_list <- lapply(1:p1, function(i){
    idx_vec <- .generate_indices_kl(bool_row = T,
                                    idx = i,
                                    p1 = p1, p2 = p2)
    .generate_objmat_kl_helper(idx_vec = idx_vec,
                               num_vec = obs_tab[idx_vec],
                               total_len = p1*p2)
  })
  col_objmat_list <- lapply(1:p2, function(j){
    idx_vec <- .generate_indices_kl(bool_row = F,
                                    idx = j,
                                    p1 = p1, p2 = p2)
    .generate_objmat_kl_helper(idx_vec = idx_vec,
                               num_vec = obs_tab[idx_vec],
...
.generate_objvec_kl <- function(num_neigh,
                                obs_tab,
                                prior_1,
                                prior_2){
  p1 <- nrow(obs_tab); p2 <- ncol(obs_tab)
  ...
  row_objvec_mat <- sapply(1:p1, function(i){
    idx_vec <- .generate_indices_kl(bool_row = T,
                                    idx = i,
                                    p1 = p1, p2 = p2)
    .generate_objvec_kl_helper(idx_vec = idx_vec,
                               num_vec = obs_tab[idx_vec],
...
.generate_objmat_kl_helper <- function(idx_vec,
                                       num_vec,
                                       total_len){
  len <- length(idx_vec)
  ...
  mat <- multiomicCCA:::.mult_vec_mat(tmp_vec, mat)
  2*mat
}

.generate_objvec_kl_helper <- function(idx_vec,
                                       num_vec,
                                       target_count,
                                       total_len){
  vec <- rep(0, total_len)
...
.generate_constr_kl <- function(obs_tab,  
                                target_count){
  p1 <- nrow(obs_tab); p2 <- ncol(obs_tab)
  total_len <- p1*p2
