...
load("../../../../out/Writeup14j/Writeup14j_citeseq_pbmc224_clisi2.RData")
...
max_val <- 1
k <- nrow(mat_c)
idx_list <- vector("list", 5)
idx_list[[1]] <- 1:k
idx_list[[2]] <- which(sort(unique(pbmc$celltype.l2)) %in% c("CD4 Naive", "Treg", "CD4 TEM", "CD4 TCM", "CD4 CTL", 
                                                             "MAIT", "CD8 Naive", "CD8 TCM", "CD8 TEM", "dnT", 
                                                             "gdT", "Eryth",
                                                             "CD4 Proliferating", "CD8 Proliferating"))
idx_list[[3]] <- which(sort(unique(pbmc$celltype.l2)) %in% c("B naive", "B intermediate", "B memory", "Plasmablast"))
idx_list[[4]] <- which(sort(unique(pbmc$celltype.l2)) %in% c("NK_CD56bright", "NK Proliferating", "NK", "ILC"))
idx_list[[5]] <- which(sort(unique(pbmc$celltype.l2)) %in% c("Platelet", "HSPC", "Doublet", "CD16 Mono", "CD14 Mono", 
                                                             "cDC1", "cDC2", "ASDC", "pDC"))
names(idx_list) <- c("All", "T cells", "B cells", "NK cells", "Monocytes")

for(kk in 1:length(idx_list)){
  png(paste0("../../../../out/figures/Writeup14j/Writeup14j_citeseq_pbmc224_clisi_", names(idx_list)[kk], ".png"), 
  ...
  for(i in idx_list[[kk]]){
    ...
    multiomicCCA:::.pie_custom(x = c(c_vec_final, rev(d_vec_final)),
  ...
  for(i in idx_list[[kk]]){
    ...
    d_vec_final <- c(d_df$value[d_idx], sum(d_df$value[-d_idx]))
    d_col_final <- c(d_df$color[d_idx], "gray")
    multiomicCCA:::.pie_custom(x = c(d_vec_final, rev(c_vec_final)),
