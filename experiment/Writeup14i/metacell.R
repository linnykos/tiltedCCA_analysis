...
.compute_subcluster_splits <- function(clustering, mat, target_k){
  stopifnot(is.list(clustering), 
            sum(sapply(clustering, length)) <= nrow(mat),
...
.compute_metacells <- function(k, mat, row_indices){
  ...
  svd_res <- multiomicCCA:::.svd_truncated(mat, K = k, symmetric = F, rescale = F, 
                                           mean_vec = T, sd_vec = T, K_full_rank = F)
