...
.compute_metacells <- function(k, 
  ...
  svd_res_1 <- multiomicCCA:::.svd_truncated(mat_1, K = min(c(k, ncol(mat_1), max_dim)), 
                                             symmetric = F, 
                                             rescale = F, 
                                             mean_vec = T, 
                                             sd_vec = F, 
                                             K_full_rank = F)
  dimred_1 <- multiomicCCA:::.mult_mat_vec(svd_res_1$u, svd_res_1$d/svd_res_1$d[1])
  
  svd_res_2 <- multiomicCCA:::.svd_truncated(mat_2, K = min(c(k, ncol(mat_2), max_dim)), 
                                             symmetric = F, 
                                             rescale = F, 
                                             mean_vec = T, 
                                             sd_vec = F, 
                                             K_full_rank = F)
  dimred_2 <- multiomicCCA:::.mult_mat_vec(svd_res_2$u, svd_res_2$d/svd_res_2$d[1])
