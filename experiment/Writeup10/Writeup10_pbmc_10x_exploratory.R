...
dim(mat_1); dim(mat_2)
svd_res_1 <- RSpectra::svds(mat_1, k = 50)
svd_res_2 <- RSpectra::svds(mat_2, k = 50) # about 10 minutes
-diff(svd_res_1$d)/svd_res_1$d[-1]; -diff(svd_res_2$d)/svd_res_2$d[-1]
rank_1 <- 17; rank_2 <- 13
png("../../out/figures/Writeup10/Writeup10_pbmc_10x_rank.png", height = 1200, width = 2500, units = "px", res = 300)
par(mfrow = c(1,2))
graphics::plot(svd_res_1$d, xlab = "Rank", ylab = "Singular value", main = "SVD for RNA", pch = 16)
# graphics::lines(rep(K+.5, 2), c(-1e5, 1e5), col = 2, lty = 2, lwd = 2)
graphics::lines(rep(rank_1+.5, 2), c(-1e5, 1e5), col = 2, lty = 2, lwd = 2)
graphics::plot(svd_res_2$d, xlab = "Rank", ylab = "Singular value", main = "SVD for ATAC", pch = 16)
# graphics::lines(rep(K+.5, 2), c(-1e5, 1e5), col = 2, lty = 2, lwd = 2)
graphics::lines(rep(rank_2+.5, 2), c(-1e5, 1e5), col = 2, lty = 2, lwd = 2)
graphics.off()
