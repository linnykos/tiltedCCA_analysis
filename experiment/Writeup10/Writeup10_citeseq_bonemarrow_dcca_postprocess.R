...
load("../../out/Writeup10_citeseq_bonemarrow_dcca.RData")
...
rownames(res$common_mat_1) <- colnames(bm)
...
zz <- extract_embedding(res, common_1 = T, common_2 = T, distinct_1 = F, distinct_2 = F, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_common_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Common view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = F, common_2 = F, distinct_1 = T, distinct_2 = T, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_distinct_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Distinct view (D-CCA)")
graphics.off()

set.seed(10)
zz <- extract_embedding(res, common_1 = T, common_2 = T, distinct_1 = T, distinct_2 = T, only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_everything_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Everything view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = T, common_2 = F, distinct_1 = F, distinct_2 = F, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_rna_common_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("RNA common view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = F, common_2 = F, distinct_1 = T, distinct_2 = F, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_rna_distinct_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("RNA distinct view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = T, common_2 = F, distinct_1 = T, distinct_2 = F, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_rna_only_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("RNA view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = F, common_2 = T, distinct_1 = F, distinct_2 = F, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_protein_common_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Protein common view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = F, common_2 = F, distinct_1 = F, distinct_2 = T, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_protein_distinct_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Protein distinct view (D-CCA)")
graphics.off()
...
zz <- extract_embedding(res, common_1 = F, common_2 = T, distinct_1 = F, distinct_2 = T, 
                        only_embedding = F)
bm[["common_factor"]] <- zz
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_protein_only_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(bm, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Protein view (D-CCA)")
graphics.off()
...
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_values.png", height = 800, width = 2000, units = "px", res = 300)
par(mfrow = c(1,3))
# cca values
...
# try computing RNA-vs-Protein weights
...
weight_mat <- t(sapply(1:n, function(i){
  ...
  names(vec) <- c("RNA", "Protein")
  vec
}))

quantile(weight_mat[,1]); quantile(weight_mat[,2])
weight_mat <- data.frame(RNA = weight_mat[,1], Protein = weight_mat[,2], cell_type = bm@meta.data[,"celltype.l2"])
...
summary_mat <- data.frame(RNA = summary_mat[,1], Protein = summary_mat[,2], cell_type = uniq_types)
...
png("../../out/figures/Writeup10/Writeup10_citeseq_bm_dcca_weights.png", height = 1500, width = 2500, units = "px", res = 300)
par(mar = c(6, 4, 3.5, 0.5))
plot(NA, xlim = c(0.5, 1.5*(len-1)), ylim = c(-1,1), xlab = "", ylab = "Ratio explained",
     xaxt = "n", main = "RNA distinct vs. Common vs. Protein distinct\nexplained variability")
