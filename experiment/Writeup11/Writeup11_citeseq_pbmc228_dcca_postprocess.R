...
load("../../../../out/Writeup11/Writeup11_citeseq_pbmc228_dcca.RData")
...
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_common_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Common view (224, D-CCA)")
graphics.off()
...
zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = F, common_2 = F, distinct_1 = T, distinct_2 = T, 
                                           only_embedding = F, vis_param = dcca_decomp$vis_param)
pbmc[["common_factor"]] <- zz
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_distinct_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Distinct view (224, D-CCA)")
graphics.off()
...
zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = T, common_2 = T, distinct_1 = T, distinct_2 = T, 
                                           only_embedding = F, vis_param = dcca_decomp$vis_param)
pbmc[["common_factor"]] <- zz
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_everything_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("Everything view (224, D-CCA)")
graphics.off()
...
zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = T, common_2 = F, distinct_1 = F, distinct_2 = F, 
                                           only_embedding = F, vis_param = dcca_decomp$vis_param)
pbmc[["common_factor"]] <- zz
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_rna_common_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("RNA common view (224, D-CCA)")
graphics.off()
...
zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = F, common_2 = F, distinct_1 = T, distinct_2 = F, 
                                           only_embedding = F, vis_param = dcca_decomp$vis_param)
pbmc[["common_factor"]] <- zz
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_rna_distinct_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("RNA distinct view (224, D-CCA)")
graphics.off()
...
zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = T, common_2 = F, distinct_1 = T, distinct_2 = F, 
                                           only_embedding = F, vis_param = dcca_decomp$vis_param)
pbmc[["common_factor"]] <- zz
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_rna_everything_umap.png", height = 1500, width = 1500, units = "px", res = 300)
plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                         repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
plot1 + ggplot2::ggtitle("RNA everything view (224, D-CCA)")
graphics.off()
...
#########################
  ...
  zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = F, common_2 = T, distinct_1 = F, distinct_2 = F, 
                                             only_embedding = F, vis_param = dcca_decomp$vis_param)
  pbmc[["common_factor"]] <- zz
  png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_protein_common_umap.png", height = 1500, width = 1500, units = "px", res = 300)
  plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                           repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
  plot1 + ggplot2::ggtitle("Protein common view (224, D-CCA)")
  graphics.off()
  ...
  zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = F, common_2 = F, distinct_1 = F, distinct_2 = T, 
                                             only_embedding = F, vis_param = dcca_decomp$vis_param)
  pbmc[["common_factor"]] <- zz
  png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_protein_distinct_umap.png", height = 1500, width = 1500, units = "px", res = 300)
  plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                           repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
  plot1 + ggplot2::ggtitle("Protein distinct view (224, D-CCA)")
  graphics.off()
  ...
  zz <- multiomicCCA::extract_umap_embedding(svd_list, common_1 = F, common_2 = T, distinct_1 = F, distinct_2 = T, 
                                             only_embedding = F, vis_param = dcca_decomp$vis_param)
  pbmc[["common_factor"]] <- zz
  png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_protein_everything_umap.png", height = 1500, width = 1500, units = "px", res = 300)
  plot1 <- Seurat::DimPlot(pbmc, reduction = 'common_factor', group.by = 'celltype.l2', label = TRUE, 
                           repel = TRUE, label.size = 2.5) + Seurat::NoLegend()
  plot1 + ggplot2::ggtitle("Protein everything view (224, D-CCA)")
  graphics.off()
...
tmp <- multiomicCCA::dcca_information_weight(mat = dcca_decomp$common_mat_1+dcca_decomp$distinct_mat_1,
                                             common_mat = dcca_decomp$common_mat_1, verbose = T, iter_max = 30)
weight_vec1 <- tmp$alpha_vec; tmp2 <- tmp
tmp <- multiomicCCA::dcca_information_weight(mat = dcca_decomp$common_mat_2+dcca_decomp$distinct_mat_2,
                                             common_mat = dcca_decomp$common_mat_2, verbose = T, iter_max = 30)
weight_vec2 <- tmp$alpha_vec
weight_mat <- data.frame(RNA = 1-weight_vec1, Protein = 1-weight_vec2, cell_type = pbmc@meta.data$celltype.l2)
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_information_rank1.png", height = 1500, width = 2500, units = "px", res = 300)
par(mfrow = c(1,1), mar = c(6, 4, 3.5, 0.5))
multiomicCCA::information_plot(weight_mat,  main = "D-CCA's weights (224, Rank-1)")
graphics.off()
...
tmp <- custom_explained_variance2(dcca_decomp, pbmc@meta.data$celltype.l2)
weight_mat <- data.frame(RNA = tmp$weight_vec_1, Protein = tmp$weight_vec_2, cell_type = sort(unique(pbmc@meta.data$celltype.l2)))
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_information_R2.png", height = 1500, width = 2500, units = "px", res = 300)
par(mfrow = c(1,1), mar = c(6, 4, 3.5, 0.5))
multiomicCCA::information_plot(weight_mat, main = "D-CCA's weights (224, R-Squared)", plot_individual = F)
graphics.off()

tmp <- custom_explained_variance(dcca_decomp)
weight_mat <- data.frame(RNA = tmp$cell_weight_vec1, Protein = tmp$cell_weight_vec2, cell_type = pbmc@meta.data$celltype.l2)
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_information_ratio.png", height = 1500, width = 2500, units = "px", res = 300)
par(mfrow = c(1,1), mar = c(6, 4, 3.5, 0.5))
multiomicCCA::information_plot(weight_mat, main = "D-CCA's weights (224, L2 ratio)")
graphics.off()
...
png("../../../../out/figures/Writeup11/Writeup11_citeseq_pbmc228_dcca_values.png", height = 800, width = 2000, units = "px", res = 300)
par(mfrow = c(1,3))
# cca values
