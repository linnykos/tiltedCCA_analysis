...
for(k in 1:3){
  ...
  png(paste0("../../../../out/figures/Writeup14p/Writeup14p_10x_greenleaf_leafplots-developmental-v2_", 
             k, ".png"),
  ...
  for(gene in genes){
    ...
    x_vec1 <- atac_mat[idx,gene]
    x_vec2 <- pred_atac_mat[idx,gene]
    y_vec1 <- rna_common[idx,gene]
    y_vec2 <- pred_rna_common[idx,gene]
    ...
    points(y = pred_rna_common[1,gene], 
           x = pred_atac_mat[1,gene],
