...
distinct_changepoint <- detect_changepoints(obj = dcca_decomp$distinct_mat_1,
                                          prin_df = prin_df,
                                          scale_factor = 2)
common_changepoint2 <- detect_changepoints(obj = dcca_decomp$common_mat_1,
                                          prin_df = prin_df,
                                          scale_factor = 1,
                                          min_len = floor(nrow(prin_df)/4))
distinct_changepoint2 <- detect_changepoints(obj = dcca_decomp$distinct_mat_1,
                                            prin_df = prin_df,
                                            scale_factor = 1,
                                            min_len = floor(nrow(prin_df)/4))
...
n <- floor(nrow(prin_df)/2)
min_length <- n/20
common_len <- sapply(common_changepoint, function(interval_mat){
  idx <- rep(FALSE, n)
  for(j in 1:nrow(interval_mat)){
    idx[interval_mat[j,"start"]:interval_mat[j,"end"]] <- TRUE
  }
  length(which(idx))
})
distinct_len <- sapply(distinct_changepoint, function(interval_mat){
  idx <- rep(FALSE, n)
  for(j in 1:nrow(interval_mat)){
    idx[interval_mat[j,"start"]:interval_mat[j,"end"]] <- TRUE
  }
