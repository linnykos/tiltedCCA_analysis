...
# now preprocess the ATAC data
# In page 44 of https://www.biorxiv.org/content/10.1101/2020.10.23.349407v2.full.pdf
# "Next,we regressed out cell total coverage"... should I do this?
...
plot1 <- Seurat::DimPlot(SNU, reduction = "umap.atac",
                         group.by = "clone")
plot1 <- plot1 + ggplot2::ggtitle("SNU (ATAC)")
plot1 <- plot1 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup14d/Writeup14d_SNU_atac.png"),
...
SNU[["cnaumap"]] <- Seurat::CreateDimReducObject(embedding = umap_res, 
                                                 key = "cnaumap_", assay = "ATAC")
plot1 <- Seurat::DimPlot(SNU, reduction = "cnaumap",
                         group.by = "clone")
plot1 <- plot1 + ggplot2::ggtitle("SNU (Copy Number)")
plot1 <- plot1 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup14d/Writeup14d_SNU_cna.png"),
                plot1, device = "png", width = 5, height = 5, units = "in")
...
plot1 <- Seurat::DimPlot(SNU, reduction = "wnn.umap",
                         group.by = "clone")
plot1 <- plot1 + ggplot2::ggtitle("SNU (WNN)")
plot1 <- plot1 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
ggplot2::ggsave(filename = paste0("../../../../out/figures/Writeup14d/Writeup14d_SNU_wnn.png"),
                plot1, device = "png", width = 5, height = 5, units = "in")
