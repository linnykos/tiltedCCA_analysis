...
ggplot2::ggsave(filename = paste0("../../../out/figures/main/citeseq_bm25_extreme_tilt09_tcca-umap_cleaned.png"),
...
multiSVD_obj3 <- tiltedCCA:::tiltedCCA_decomposition(input_obj = multiSVD_obj3,
                                                     verbose = 1)
set.seed(10)
bm[["common_tcca3"]] <- tiltedCCA:::create_SeuratDim(input_obj = multiSVD_obj3,
                                                    what = "common",
                                                    aligned_umap_assay = "rna.umap",
                                                    seurat_obj = bm,
                                                    seurat_assay = "RNA",
...
plot1 <- Seurat::DimPlot(bm, reduction = "common_tcca3",
                         group.by = "celltype.l2", 
                         cols = col_palette)
plot1 <- plot1 + Seurat::NoLegend() + Seurat::NoAxes()
plot1 <- plot1 + ggplot2::ggtitle("")
plot1 <- plot1 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
ggplot2::ggsave(filename = paste0("../../../out/figures/main/citeseq_bm25_extreme_tilt01_tcca-umap_cleaned.png"),
                plot1, device = "png", width = 3, height = 3, units = "in",
                dpi = 500)