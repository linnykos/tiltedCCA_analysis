...
variable_selection_res <- tiltedCCA:::postprocess_variable_selection(
  ...
  input_mat = t(as.matrix(bm[["AB"]]@scale.data)),
...
set.seed(10)
bm <- Seurat::FindMultiModalNeighbors(bm, 
                                        reduction.list = list("pca", "apca2"),
                                        dims.list = list(1:40, 1:(ncol(bm[["apca2"]]@cell.embeddings)-1)), 
                                        modality.weight.name = "RNA.weight",
                                        weighted.nn.name = "weighted.nn2")
set.seed(10)
bm <- Seurat::RunUMAP(bm, nn.name = "weighted.nn2", 
...
save(variable_selection_res, bm, 
     ...
     file = "../../../out/main/abseq_bm97_varSelect.RData")
...
ggplot2::ggsave(filename = paste0("../../../out/figures/main/abseq_bm97_varSelect_adt-umap.png"),
                plot2, device = "png", width = 9, height = 5, units = "in")

plot3 <- Seurat::DimPlot(bm, reduction = "wnn2.umap",
                         group.by = "ct", label = TRUE,
                         repel = TRUE, label.size = 2.5,
                         cols = col_palette,
                         raster = FALSE)
...
ggplot2::ggsave(filename = paste0("../../../out/figures/main/abseq_bm97_varSelect_wnn-umap.png"),
                plot3, device = "png", width = 9, height = 5, units = "in")
...
plot3 <- plot3 + ggplot2::theme(legend.text = ggplot2::element_text(size = 5))
ggplot2::ggsave(filename = paste0("../../../out/figures/main/abseq_bm97_varSelect_consensusPCA-umap.png"),
                plot3, device = "png", width = 9, height = 5, units = "in")
