...
vec1 <- multiSVD_obj$svd_1$u[,1]
vec2 <- multiSVD_obj$svd_2$u[,1]

vec1 <- vec1 - median(vec1); vec1 <- vec1/(diff(quantile(vec1, probs = c(0.02, 0.98))))
vec2 <- vec2 - median(vec2); vec2 <- vec2/(diff(quantile(vec2, probs = c(0.02, 0.98))))

vec1 <- vec1 - median(vec1)
vec2 <- vec2 - median(vec2)

png("../../../out/figures/main/citeseq_bm25_pca_leading-scores.png",
    height = 1200, width = 1200, units = "px", res = 500)
par(mar = c(0.5, 4, 0.5, 0.5), bg = NA)
plot(NA, ylim = range(c(vec1, vec2)), xlim = c(-.5, 1.5), 
     ylab = "Leading principal score", xlab = "", bty = "n", xaxt = "n",
     yaxt = "n")
axis(side = 2, labels = F)
points(y = vec1, x = rep(0, length(vec1)) + runif(length(vec1), min = -.3, max = .3),
       pch = 16, col = col_vec, cex = 0.25)
