...
rna_mat <- multiSVD_obj$common_mat_1[cell_idx[order_vec],] + multiSVD_obj$distinct_mat_1[cell_idx[order_vec],]
...
rna_mat <- rna_mat[,mentioned_genes]
...
all(rownames(atac_mat) == rownames(rna_mat))
...
for(k in 1:3){
  ...
  png(paste0("../../../out/figures/main/10x_greenleaf_development_leafplots_ATAC-RNA_lineage1_", 
             k, ".png"),
...
for(k in 1:3){
  ...
  png(paste0("../../../out/figures/main/10x_greenleaf_development_timeseries_ATAC-RNA_lineage1_", 
             k, ".png"),
      height = 3500, width = 3000, units = "px", res = 300)
  par(mfrow = c(5,4), mar = c(4,4,4,0.5))
  for(gene in genes){
